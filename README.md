...
### Авторизация из вашего браузера (CDP)

Вместо изолированного Chromium прокси может использовать **ваш уже запущенный Chrome**, где вы залогинены в Qwen Chat. Удобно, когда в изолированном браузере мешает капча или анти-бот-проверка.

1. Запустите Chrome с портом отладки. Флаг `--user-data-dir` создаёт отдельный профиль только под Qwen, чтобы не закрывать ваш основной Chrome:

   **Windows (PowerShell):**

   ```powershell
   & "C:\Program Files\Google\Chrome\Application\chrome.exe" --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\QwenChromeProfile"
   ```

   Без `--user-data-dir` Chrome подключится к вашему основному профилю — но только если все остальные окна Chrome закрыты (иначе порт отладки не откроется).

2. В открывшемся окне войдите в `https://chat.qwen.ai`.

3. Укажите адрес отладки и запустите прокси:
...
Прокси подключится к вашему Chrome, возьмёт токен из вашей сессии и не станет открывать изолированный браузер. При остановке прокси ваш Chrome **не закрывается** (используется `disconnect`). В этом режиме `npm run auth` и `tokens.json` не нужны.

**Безопасность:** порт `9222` слушается только на `127.0.0.1`, то есть доступен лишь локальным процессам — не пробрасывайте его наружу. Любой процесс с доступом к порту получает полный контроль над этим окном Chrome.

> В CDP-режиме рабочие запросы открывают вкладки `chat.qwen.ai` в этом окне Chrome — это нормально. Ротация нескольких аккаунтов здесь не используется: берётся токен текущей сессии.
