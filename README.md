...
После запуска откройте `http://localhost:3264/` (или `/dashboard`) — встроенная панель управления:

- **Чат** — стриминг ответов, многоходовой диалог, рендер Markdown, прикрепление картинок и документов.
- **Картинки / Видео** — генерация через Qwen Chat с предпросмотром и скачиванием.
- **Обзор** — как подключиться (Base URL / ключ / модель, сниппеты cURL и Python), список моделей и управление аккаунтами Qwen: проверка валидности, лимиты/срок токена, обновление токена (relogin), добавление и удаление.

Управление аккаунтами и скачивание медиа доступны только с `localhost`.
